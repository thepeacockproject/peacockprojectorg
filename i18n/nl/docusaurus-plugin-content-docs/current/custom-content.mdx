...
* **Custom Peacock Escalations**
  * Peacock supports custom (Peacock original) escalations. Deze escalations zijn standaard beschikbaar in het spel. Je kan ook enkele aangepaste escalations zien in de [Roadmaps](https://thepeacockproject.org/wiki/roadmaps).

* **Official Server Escalations**
    * Peacock also supports some official server escalations, although not all are ported. Deze escalations zijn standaard beschikbaar in het spel.
* **Legacy Escalations**
    * Peacock supports Hitman 2016 escalations that have not been ported to Hitman 2 or 3 (legacy escalations).
    * Om legacy escalations te installeren, zoek `legacy in: plugin-releases` in de Peacock Discord. Download `LegacyEscalationsPlugin.js`, and move this file to the same folder that `Start Server.cmd` and `peacockpatcher.exe` are in.
