...
* **User Created, Peacock Contracts** 
  ...
  * How to Play: Now go ```Game Modes``` -> ```Contracts``` -> ```My Contracts``` in-game to play.
  * How to Make Your Own Peacock Contracts: If you create a contract in-game while in Peacock server, your contract will be saved as an ```.ocre``` file in ```...Peacock-vx.x.x/Peacock-vx.x.x/Contracts``` folder. Be sure to share your ```.ocre``` file in Peacock Discord's ```share-contracts``` channel if you want others to play your custom Peacock contract.
